...
async function simsimi(a, b, c) {
  ...
  try {
    var { data: j } = await axios({
      url: `https://ms-ykuk.onrender.com/sim?type=ask&ask=${g(a)}`,
      method: "GET",
    });
  ...
  switch (args[0]) {
    case "on":
      return global.manhG.simsimi.has(threadID)
        ? body("[ 𝐒𝐈𝐌 ] - Bật gì tận 2 lần hả em.")
        : (global.manhG.simsimi.set(threadID, messageID),
          body("[ 𝐒𝐈𝐌 ] - Bật sim thành công."));
    case "off":
      return global.manhG.simsimi.has(threadID)
        ? (global.manhG.simsimi.delete(threadID),
          body("[ 𝐒𝐈𝐌 ] - Tắt sim thành công."))
        : body("[ 𝐒𝐈𝐌 ] - Tao đang phấn khởi tắt cái qq.");
    default:
